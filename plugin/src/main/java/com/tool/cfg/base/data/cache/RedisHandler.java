package com.tool.cfg.base.data.cache;
...
public class RedisHandler<T> {
    ...
    private Logger logger = LogManager.getLogger(RedisHandler.class);
     ...
     * @return 缓存的对象
     */
    public <T> boolean setCacheObject(final String key, T value, Long expireTime) {
    ...
    public void keepExpire(final String key, Long expireTime) {
        try {
            ValueOperations<String, T> operation = redisTemplate.opsForValue();
            redisTemplate.expire(key, expireTime, TimeUnit.SECONDS);
        } catch (Exception e) {
            logger.error("redis keepExpire error:" + e.getMessage());
            e.printStackTrace();
        }
    }
     ...
     * @return 缓存键值对应的数据
     */
    public <T> boolean hasKey(String key/*,ValueOperations<String,T> operation*/) {
       // ValueOperations<String, T> operation = redisTemplate.opsForValue();
